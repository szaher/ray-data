...
description: "How Python's Global Interpreter Lock limits parallelism"
quiz:
...
`} fallback="The GIL forces all Python threads through a single lock, so only one core runs Python code regardless of thread count." />

## The multiprocessing workaround

Python's `multiprocessing` module bypasses the GIL by creating separate processes, each with its own Python interpreter and memory space. This has costs, though.

The serialization problem: to send data between processes, Python must *serialize* (pickle) it. That means converting it to bytes, copying it, then deserializing on the other side. For a 1 GB DataFrame:
...
`} fallback="Multiprocessing requires costly serialization and copying, doubling memory and adding seconds of overhead per transfer." />
...
Ray gives you the parallelism of multiprocessing without the serialization overhead.
